...
export const AnatomyExplorer: React.FC<Props> = ({ scale, selectedId, onSelect, onScaleChange }) => {
  ...
  return (
    <div className="pointer-events-none absolute inset-x-3 top-20 z-20 flex flex-col items-center gap-2">
      <div className="pointer-events-auto panel w-full max-w-xl rounded-xl px-3 py-2.5 shadow-2xl shadow-black/40">
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-1 text-[10px] text-slate-400"><span>Body</span><ChevronRight className="h-3 w-3"/><span>Systems</span><ChevronRight className="h-3 w-3"/><span>Organs</span></div>
        </div>
      </div>
