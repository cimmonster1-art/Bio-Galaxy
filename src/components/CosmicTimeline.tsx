...
export const CosmicTimeline: React.FC<Props> = ({ scale, onSelectScale, onClose }) => (
  <section className="workspace-overlay workspace-eras" aria-label="Eras browser">
    ...
    <ol className="grid min-h-0 flex-1 grid-cols-2 gap-px overflow-y-auto scroll-thin bg-white/5 p-px sm:grid-cols-3 lg:grid-cols-4">{HISTORY_CHAPTERS.map((era, index) => {
      const selected = scale === era.scale;
      return <li key={era.id}><button type="button" onClick={() => onSelectScale(era.scale)} className={`group flex h-full min-h-28 w-full flex-col items-start bg-[#07101d] p-4 text-left transition hover:bg-[#0a1727] ${selected ? 'ring-1 ring-inset ring-cyan-400/50' : ''}`}><span className="mb-4 font-mono text-[10px] text-slate-500">{String(index + 1).padStart(2, '0')} · {era.time}</span><span className="text-[13px] font-semibold text-slate-100 group-hover:text-cyan-100">{era.label}</span><span className="mt-1 text-[10px] leading-relaxed text-slate-500">{era.detail}</span><span className={`mt-auto h-0.5 w-full origin-left rounded-full transition ${selected ? 'bg-cyan-300' : 'scale-x-25 bg-white/10 group-hover:scale-x-100 group-hover:bg-cyan-400/50'}`}/></button></li>;
    })}</ol>
  </section>
);
