...
import { resolveCelestial, NAMED_STAR_OBJECTS, DEEP_SKY_OBJECTS } from './celestial';
...
export function resolveObject(id: string): BioObject | undefined {
  ...
  return BIO_OBJECTS[id] ?? taxonObjects[id] ?? ANATOMY_OBJECTS[id] ?? cosmicObjects[id] ?? cosmicStructures[id] ?? atomObjects[id] ?? biomeObjects[id] ?? ecosystemObjects[id] ?? ecologyObjects[id] ?? resolveCelestial(id) ?? tissueObjects[id] ?? moleculeObjects[id];
}
